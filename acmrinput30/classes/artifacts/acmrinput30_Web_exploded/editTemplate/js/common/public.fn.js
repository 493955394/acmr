define(function(require, exports, module) {
	...
	pub.initArgTree = function(wd) {
		...
		$.ajax({
			url: rootPath + "/templatecenter/TemplateHandel.htm?m=getResultLeftTree",
			type: "post",
			data: {
				"wd": wd,
				"proCode": ""
	...
	pub.scroll = function(s, e) {
		$(".scroll-container").scroll(function() {
			...
			if (parseInt(maxRow1) > parseInt(maxRow)) {
				ss.batchFillBg("1", "rgb(184, 204, 228)", [s + maxRow, e + maxRow1]);
	...
	pub.refreshFilter = function() {
		$.ajax({
			url: rootPath + '/templatecenter/DistinguishArea.htm?m=getFilterName&modId=' + modId + '&templateCode=' + window.SPREADSHEET_AUTHENTIC_KEY,
			type: "post",
			dataType: "json",
			success: function(d) {
				if (d.returncode === 200) {
	...
	pub.refreshFilterBook = function() {
		$.ajax({
			url: rootPath + '/bookoffice/BookDistinguishArea.htm?m=getFilterName&modId=' + modId + '&templateCode=' + window.SPREADSHEET_AUTHENTIC_KEY,
			type: "post",
			dataType: "json",
			success: function(d) {
				if (d.returncode === 200) {
	...
	$(document).on("click", ".iSearchList .item", function() {
		...
		$.ajax({
			url: rootPath + '/templatecenter/TemplateHandel.htm?m=positionTree&wd=' + wd + '&keycode=' + code,
			type: 'post',
			dataType: "json",
			beforeSend: function() {
				var t = box.scrollTop();
	...
	$(document).on("click", ".iSearchBtn", function() {
		...
		if ($.trim(text) !== "") {
			$.ajax({
				url: rootPath + '/templatecenter/template.htm?m=find&wd=' + wd + '&text=' + text,
				type: 'post',
				dataType: "json",
				success: function(data) {
					...
					if (data.success === "true" && l) {
						...
						for (i = 0; i < l; i++) {
							$list.append('<div class="item" code="' + data.results[i].value + '" title="' + data.results[i].path + '" wd="' + wd + '">' + data.results[i].name + '</div>');
						}
					} else {
