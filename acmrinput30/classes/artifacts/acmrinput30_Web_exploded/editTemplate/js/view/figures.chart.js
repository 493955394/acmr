define(function(require, exports, module) {
	...
	$(document).on("click", "#saveFigures", function() {
		...
		if (figuresName !== oldFiguresName) {
			$.ajax({
				url: rootPath + "/bookoffice/BookCalCheckMemo.htm?m=echeckName&modId=" + modId + "&templateCode=" + window.SPREADSHEET_AUTHENTIC_KEY + "&name=" + figuresName,
				type: "post",
				async: false,
				dataType: "json",
		...
		$.ajax({
			url: rootPath + "/bookoffice/BookCalCheckMemo.htm?m=eadd&modId=" + modId,
			type: "post",
			data: {
	...
	function getFiguresList() {
		$.ajax({
			url: rootPath + "/bookoffice/BookCalCheckMemo.htm?m=efind&modId=" + modId + "&templateCode=" + window.SPREADSHEET_AUTHENTIC_KEY,
			type: "post",
			dataType: "json",
			success: succ
		});
	...
	function removeFiguresList(name) {
		$.ajax({
			url: rootPath + "/bookoffice/BookCalCheckMemo.htm?m=edel&modId=" + modId + "&templateCode=" + window.SPREADSHEET_AUTHENTIC_KEY + "&name=" + name,
			type: "post",
			dataType: "json",
			success: function(data) {
				if (data.returncode === 200) {
