define(function(require, exports, module) {
	...
	$(document).on("click", "#addArgument", function() {
		...
		$.ajax({
			url: rootPath + "/bookoffice/BookDistinguishArea.htm?m=checkName&modId=" + modId + "&templateCode=" + window.SPREADSHEET_AUTHENTIC_KEY + "&name=" + areaName,
			type: "post",
			async: false,
			dataType: "json",
	...
	$(document).on("click", "#saveArea", function() {
		...
		$.ajax({
			url: rootPath + "/bookoffice/BookDistinguishArea.htm?m=add",
			type: "post",
			data: {
	...
	function setJson() {
		...
		if (isFilter === "1") {
			...
			if (filterName !== oldFilterName) {
				$.ajax({
					url: rootPath + "/bookoffice/BookDistinguishArea.htm?m=checkFname&modId=" + modId + "&templateCode=" + window.SPREADSHEET_AUTHENTIC_KEY + "&name=" + filterName,
					type: "post",
					async: false,
					dataType: "json",
					success: function(data) {
	...
	function getAreaList() {
		$.ajax({
			url: rootPath + "/bookoffice/BookDistinguishArea.htm?m=find&modId=" + modId + "&templateCode=" + window.SPREADSHEET_AUTHENTIC_KEY,
			type: "post",
			dataType: "json",
			success: succ
		});
	...
	function removeArea(name) {
		$.ajax({
			url: rootPath + "/bookoffice/BookDistinguishArea.htm?m=del&modId=" + modId + "&templateCode=" + window.SPREADSHEET_AUTHENTIC_KEY + "&name=" + name,
			type: "post",
			dataType: "json",
			success: function(data) {
				if (data.returncode === 200) {
	...
	function getOutArgDropdown() {
		...
		var setting = {
			async: {
				enable: true,
				url: rootPath + '/bookoffice/BookTemplateHandel.htm?m=getResultLeftTree&wd=' + wd,
