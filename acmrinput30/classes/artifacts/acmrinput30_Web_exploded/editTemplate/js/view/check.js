define(function(require, exports, module) {
	...
	$(document).on("click", "#saveCheck", function() {
		...
		if(checkName !== oldCheckName){
			$.ajax({
				url: rootPath + "/templatecenter/calCheckMemo.htm?m=ccheckName&modId="+modId+"&templateCode=" + window.SPREADSHEET_AUTHENTIC_KEY + "&name=" + checkName,
				type: "post",
				async: false,
				dataType: "json",
		...
		$.ajax({
			url: rootPath + "/templatecenter/calCheckMemo.htm?m=cadd&modId="+modId,
			type: "post",
			data: {
	...
	function getCheckList() {
		$.ajax({
			url: rootPath + "/templatecenter/calCheckMemo.htm?m=cfind&modId="+modId+"&templateCode=" + window.SPREADSHEET_AUTHENTIC_KEY,
			type: "post",
			dataType: "json",
			success: succ
		});
	...
	function removeCheckList(name) {
		$.ajax({
			url: rootPath + "/templatecenter/calCheckMemo.htm?m=cdel&modId="+modId+"&templateCode=" + window.SPREADSHEET_AUTHENTIC_KEY + "&name=" + name,
			type: "post",
			dataType: "json",
			success: function(data) {
				if (data.returncode === 200) {
