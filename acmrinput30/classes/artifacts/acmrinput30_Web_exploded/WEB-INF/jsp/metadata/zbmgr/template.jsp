...
<html>
	<head>
		<meta charset="UTF-8">
		<title>${projectTitle}-指标管理-模板详情</title>
...
<body>
	            ...
	            <ul class="pagination J_template_pagination">
	            	...
	            	<c:forEach begin="2" end="4" varStatus="i">
		            	<c:if test="${page.pageNum+i.index-1 <= page.totalPage}">
		            		<li><a href="${ctx}/metadata/zbmgr.htm?m=getTemplateData&code=${code}&pageNum=${page.pageNum+i.index-1}">${i.index}</a></li>
						</c:if>
	            	</c:forEach>
	            	<c:choose>
	            		...
	            		<c:otherwise>
			            	<li><a href="${ctx}/metadata/zbmgr.htm?m=getTemplateData&code=${code}&pageNum=${page.pageNum+1}">下一页</a></li>
			            	<li><a href="${ctx}/metadata/zbmgr.htm?m=getTemplateData&code=${code}&pageNum=${page.totalPage}">末页</a></li>
	            		</c:otherwise>
	            	</c:choose>
	            </ul>
			</div>
	...
	<script type="text/javascript">
		seajs.use('${ctx}/js/func/metadata/zbmgr/template');
	</script>
</body>
</html>