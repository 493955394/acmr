...
<table class="table table-striped table-hover J_mdate_table">
	<colgroup>
		<col width="5%"/>
		<col width="10%"/>
		<col width="25%"/>
	...
	<tbody>
		...
		<c:forEach items="${page.data}" var="list" varStatus="status">
			<tr>
				...
				<td><a href="${ctx}/metadata/mdate.htm?m=getById&id=${list.code}" >${list.cname}</a></td>
				<td>${list.procode}</td>
				<td>
					<a href="${ctx}/metadata/mdate.htm?m=turnToEdit&id=${list.code}" class="btn-opr J_opr_edit">编辑</a>
...
<div class="toolbar-right">
	<ul class="pagination J_mdate_pagination">${page}</ul>  	
</div>