define(function(require,exports,module){
	...
	var setting = {
		async: {
			enable: true,
			url: common.rootPath+'metadata/datetype.htm?m=findDtTree&st='+st,
	...
	function clickEvent(event, treeid, treeNode) {
		//queryData(common.rootPath+"datetype.htm?m=find", common.formatData('id',treeNode.id));
		...
		$.pjax({
			url: common.rootPath+"metadata/datetype.htm?m=find"+str+ common.formatData('id',treeNode.id)+"&treeList="+initTreePara,
			container: '.J_datetype_data_table'
		});
	...
	var rootNode = [{"id":"","name":"时间类型树", "open":"true", "isParent":"true"}];
	...
	function initFileUpload(){
		$('#J_fileupload',document).fileupload({
			url: common.rootPath+'metadata/datetype.htm?m=matchingDetails',
	        dataType: 'json',
	...
	$(document).on('click','.J_btn_md_export',function(event){
		event.preventDefault();
		window.location.href=common.rootPath+"metadata/datetype.htm?m=mdExport&id="+idColumn+"&uuid="+uuid;
	});
	...
	$(document).on('click', '#mouldDownload', function(event) {
		window.location.href = common.rootPath+"metadata/datetype.htm?m=templateDownload";
	});
	...
	function importData(){
		$('#J_fileupload_import',document).fileupload({
			url: common.rootPath+'metadata/datetype.htm?m=importData',
	        dataType: 'json',
	        done: function (e, data) {
	        	var result = data.result;
	            if(result.returncode == 200){
	...
	$(document).on('click','.J_btn_id_export',function(event){
		event.preventDefault();
		window.location.href=common.rootPath+"metadata/datetype.htm?m=impErrInfoExport&uuid="+uuid;
	});
	...
	$(document).on('click', '#mouldDownload2', function(event) {
		window.location.href = common.rootPath+"metadata/datetype.htm?m=templateDownload2";
	});
	...
	$(document).on('click','.J_opr_del',function(event){
		...
		$.ajax({
			url:common.rootPath+'metadata/datetype.htm?m=toDelete',
			data:"delId=" + delId,
			type:'post',
			dataType:'json',
			timeout:10000,
	...
	$(document).on('click','.J_All_del',function(event){
		...
		$.ajax({
			url:common.rootPath+'metadata/datetype.htm?m=deleteAll',
			data:"ids=" + delIds,
			type:'post',
			dataType:'json',
			timeout:10000,
	...
	$(document).on('click','.J_opr_moveup',function(event){
		...
		$.ajax({
			url:common.rootPath+'metadata/datetype.htm',
			type:'get',
			dataType:'json',
			data:'m=move&currentId='+currentId+'&siblingsId='+siblingsId,
			success:function(data){
				if(data.returncode == 200){
					common.cancalCheck('.J_datetype_table input[type="checkbox"]',delIds);	
//					var tempHTML = '';
...
//						$(self).parents('tbody').children('tr:eq(0)').before(tempHTML);
//					}
//					else if(indexVal > 1 && indexVal< $('.J_datetype_table tbody').children('tr').length-1){
//						$(self).parents('tbody').children('tr:eq('+(indexVal-2)+')').after(tempHTML);
//					}
//					else if(indexVal==$('.J_datetype_table tbody').children('tr').length-1){
//						var spHTML='',
//						HTML='',
//						targetHTML='';
...
//					$(self).parents('tr:eq(0)').remove();
	...
	$(document).on('click','.J_opr_code',function(event){
		...
		var flag = false;
		$.ajax({
			url:common.rootPath+"metadata/datetype.htm?m=checkHasParent&code="+currentCode,
			type:'post',
			timeout:5000,
			dataType:'json',
			async: false,
		...
		$.pjax({
			url: common.rootPath+"metadata/datetype.htm?m=find&id="+currentCode,
			container: '.J_datetype_data_table'
		});
		$(document).on('pjax:success', function() {
		  delIds = [];
	...
	$(document).on('click','.J_export_data',function(event){
		...
		if(delIds.length>0){//有没有被选中的指标
			window.location.href=common.rootPath+"metadata/datetype.htm?m=exportAllData&ids="+delIds;
		}else if(isMove){
			window.location.href=common.rootPath+"metadata/datetype.htm?m=exportData&id="+treeNodeId+"&procodeName="+treeNodeName;
		}else{
			if(searchField.length>0){
				searchPara = searchField;
			}
			window.location.href=common.rootPath+"metadata/datetype.htm?m=exportSearchData&procodeName="+treeNodeName+searchPara;
		}
	});
	...
	$(document).on('click','.J_add_data',function(event){
		event.preventDefault();
		window.location.href=common.rootPath+"metadata/datetype.htm?m=toAdd&code=" + treeNodeId + "&cname=" + treeNodeName;
	});
	
	/**
	 *提交选择的checkebox(默认所有)
	 */
	$(document).on('click','.J_datetype_table>thead>tr>th>input[type="checkbox"]',function(){
		var self = this;
		delIds = common.checkAll('.J_datetype_table>tbody input[type="checkbox"]',$(self).prop('checked'),delIds);
	});
	$(document).on('click','.J_datetype_table>tbody input[type="checkbox"]',function(){
		var self = this,
		...
		if(delIds.length === $('.J_datetype_table>tbody input[type="checkbox"]').length){
			$('.J_datetype_table>thead>tr>th>input[type="checkbox"]').attr('checked',true);
		}else{
			$('.J_datetype_table>thead>tr>th>input[type="checkbox"]').attr('checked',false);
		}
	});
	...
	function formatBackTable(data){
		if(data.returncode == 200){
			...
			if(len === 0){
				...
				$('.J_datetype_table > tbody').html(trs);
				return;
			}
			for (i; i < len; i++) {
				...
				trs +='<td><a href="'+common.rootPath+'metadata/datetype.htm?m=getById&id='+list[i].code+'" >'+list[i].cname+'</a></td>';
				trs +='<td>'+list[i].unitcode+'</td>';
				trs +='<td>'+list[i].procode+'</td>';
				trs +='<td>'+list[i].ccname+'</td>';
				trs +='<td><a href="'+common.rootPath+'metadata/datetype.htm?m=turnToUpdate&id='+list[i].code+'" class="btn-opr J_opr_edit">编辑</a>';
				if(isMove){
					if(i==0){
				...
				trs +='<a href="'+common.rootPath+'metadata/datetype.htm?m=delete&id='+list[i].code+'" class="btn-opr J_opr_del" id='+list[i].code+'>删除</a>';
				trs +='</td></tr>';
			}
			$('.J_datetype_table > tbody').html(trs);
		}
	 ...
	 * 翻页操作
	 */
	$(document).pjax('.J_datetype_pagination a', '.J_datetype_data_table');
