...
define(function(require,exports,module){
	...
	var setting = {
		async: {
			enable: true,
			url: common.rootPath+'metadata/zbmgr.htm?m=findZbTree&st='+st,
	...
	function clickEvent(event, treeid, treeNode) {
		...
		$.pjax({
			url: common.rootPath+"metadata/zbmgr.htm?m=query"+str+ common.formatData('id',treeNode.id),
	...
	var rootNode = [{"id":"","name":"指标树", "open":"true", "isParent":"true"}];
...
//			common.commonTips("格式错误");
		...
		$.ajax({
			url:common.rootPath+'metadata/zbmgr.htm',
			type:'post',
			dataType:'json',
...
//					common.commonTips("格式错误");
		...
		$.pjax({
			url: common.rootPath+"metadata/zbmgr.htm?m=advQuery&searchSQL="+advVal,
			container: '.J_zbmgr_data_table'
		});
		$(document).on('pjax:success', function() {
	...
	function fileUpload(){
		$('#J_import_fileupload',document).fileupload({
			url: common.rootPath+'metadata/zbmgr.htm?m=upload',
			type: 'post',
	        dataType: 'json',
	...
	$(document).on('click','.J_opr_del',function(event){
		...
		$.ajax({
			url:common.rootPath+'metadata/zbmgr.htm?m=toDelete',
			data:"delId=" + delId,
			type:'post',
			dataType:'json',
			timeout:10000,
	...
	$(document).on('click','.J_All_del',function(event){
		...
		$.ajax({
			url:common.rootPath+'metadata/zbmgr.htm?m=toDeleteAll',
			data:"ids=" + delIds,
			type:'post',
			dataType:'json',
			timeout:10000,
	...
	$(document).on('click','.J_export_data',function(event){
		...
		if(delIds.length>0){//有没有被选中的指标
			window.location.href=common.rootPath+"metadata/zbmgr.htm?m=exportAllData&ids="+delIds;
		}else if(isMove){//是否为普通的数据查看
			window.location.href=common.rootPath+"metadata/zbmgr.htm?m=exportData&id="+treeNodeId+"&procodeName="+treeNodeName;
		}else if(isAdvQuery){//是否为高级查询
			var hcurrentVal = $(".J_hpriview_sentence").val();
			window.location.href=common.rootPath+"metadata/zbmgr.htm?m=exportAdvQueryData&searchSQL="+hcurrentVal;
		}else{//表示进入搜索
			if(searchField.length>0){
				searchPara = searchField;
			}
			window.location.href=common.rootPath+"metadata/zbmgr.htm?m=exportSearchData&procodeName="+treeNodeName+searchPara;
		}
	});
	...
	function initFileUpload(){
		$('#J_fileupload',document).fileupload({
			url: common.rootPath+'metadata/zbmgr.htm?m=matchingDetails',
	        dataType: 'json',
	        done: function (e, data) {
	...
	$(document).on('click','.J_btn_md_export',function(event){
		event.preventDefault();
		window.location.href=common.rootPath+"metadata/zbmgr.htm?m=mdExport&id="+idColumn+"&uuid="+uuid;
	});
	...
	function importData(){
		$('#J_fileupload_import',document).fileupload({
			url: common.rootPath+'metadata/zbmgr.htm?m=importData',
	        dataType: 'json',
	        done: function (e, data) {
	        	var result = data.result;
	            if(result.returncode == 200){
	...
	$(document).on('click','.J_btn_id_export',function(event){
		event.preventDefault();
		window.location.href=common.rootPath+"metadata/zbmgr.htm?m=impErrInfoExport&uuid="+uuid;
	});
	...
	function importNewData(){
		$('#J_fileupload_import',document).fileupload({
			url: common.rootPath+'metadata/zbmgr.htm?m=importEditData',
	        dataType: 'json',
	        done: function (e, data) {
	        	var result = data.result;
	            if(result.returncode == 200){
	...
	$(document).on('click', '#mouldDownload3', function(event) {
		window.location.href = common.rootPath+"metadata/zbmgr.htm?m=templateDownload3";
	});
	...
	$(document).on('click','.J_Add',function(event){
		event.preventDefault();
		window.location.href=common.rootPath+"metadata/zbmgr.htm?m=toAdd&procodeId=" + treeNodeId + "&procodeName=" + treeNodeName;
	});
	/**
	 *根据code下转 
	 */
	$(document).on('click','.J_opr_code',function(event){
		...
		var flag = false;
		$.ajax({
			url:common.rootPath+"metadata/zbmgr.htm?m=checkHasParent&code="+currentCode,
			type:'post',
			timeout:5000,
			dataType:'json',
			async: false,
		...
		$.pjax({
			url: common.rootPath+"metadata/zbmgr.htm?m=query&id="+currentCode,
			container: '.J_zbmgr_data_table'
		});
		$(document).on('pjax:success', function() {
		  delIds = [];
	...
	function formatBackTable(data){
		if(data.returncode == 200){
			...
			for (i; i < len; i++) {
				...
				trs +='<td><a href="'+common.rootPath+'metadata/zbmgr.htm?m=getDataById&id='+list[i].code+'" >'+list[i].cname+'</a></td>';
				trs +='<td>'+list[i].unitcode+'</td>';
				trs +='<td>'+list[i].procode+'</td>';
				trs +='<td>'+list[i].ccname+'</td>';
				trs +='<td><a href="'+common.rootPath+'metadata/zbmgr.htm?m=toEdit&id='+list[i].code+'" class="btn-opr J_opr_edit">编辑</a>';
