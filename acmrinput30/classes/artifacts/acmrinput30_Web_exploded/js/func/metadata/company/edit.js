...
//		
		var next = false;
		$.ajax({
			url: common.rootPath+"metadata/company.htm?m=querySysnName&synonym="+$('[name=synonym]').val()+"&code="+$('input[name="code"]').val(),
			type: 'get',
			async: false,
			dataType: 'json',
...
//	});
	...
	function checkProcodeAsyncExist(currentVal,checkDelegate){
		var flag = false;
		$.ajax({
			url: common.rootPath+'metadata/company.htm?m=checkProCode',
			timeout: 5000,
			type: 'post',
			async: false,
