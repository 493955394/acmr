define(function(require, exports, module) {
	...
	if( $('#isPower').attr('powertype') === '1' ){
		var arrPowerList = JSON.parse( $('#isPower').val());
		
		for(var c=0;c < arrPowerList.length; c++){
			var usersHtml = '';
			var usersValue = '';
			var id = new Date().getTime();
			var html = '<div class="J_powerItem"><label class="control-label vtop">组织与用户：</label>';
			...
			for(var f=0;f < arrPowerList[c].childs.length; f++){
				if(f === 0){
					usersValue += arrPowerList[c].childs[f].code;
				}else{
					usersValue += ',' + arrPowerList[c].childs[f].code;
				}
				usersHtml += '<a class="ui label transition visible" data-value="' + arrPowerList[c].childs[f].code + '"  data-type="' + arrPowerList[c].childs[f].type + '" style="display: inline-block ! important;">' + arrPowerList[c].childs[f].name + '<i class="delete icon"></i></a>';
			}
			$("#dropdown" + id + " input").val(usersValue);
			$("#dropdown" + id + " .dropdown.icon").after(usersHtml);
			
			$(".roles" + id).append($(".publicSelect").html());
			$(".roles" + id).val( arrPowerList[c].rols );
		}
	}
