package com.acmr.dao.oracle.theme;
...
public class OraThemeDaoImpl implements IThemeDao {
	...
	public ThemePo getByCode(String code) {
		...
		try {
			...
			while (rs1.next()) {
				...
				theme.setPhoto(rs1.getString("photo"));
				theme.setVisible(rs1.getString("visible"));
				theme.setSortcode(rs1.getString("sortcode"));
				result.add(theme);
			}
		} catch (Exception e1) {
			e1.printStackTrace();
	...
	public List<Lanmu> getLanmu() {
		String sql = "select distinct dbcode,t.code,t2.cname from ACMRNDATA_PUB_V2.TB_WEB_LANMUDATA_DB t left join ACMR_PUB_V2.TB_DATA_DATABASE t2 on  t2.code = t.dbcode where sort='jd' and t.code not in (select code from acmrndata_pub_v2.tb_web_topmenu)";
	...
	public List<String> getMenuCode() {
		...
		sql.append("select * from ACMRNDATA_PUB_V2.TB_WEB_TOPMENU_RIGHT where users like ? ");
		parm.add("%" + currentUser.getUserid() + ",%");
		...
		// StringBuffer ros = new StringBuffer();
		// for (int i = 0; i < roles.size(); i++) {
		// ros.append("'");
		// ros.append(roles.get(i));
		// ros.append("',");
		// }
		// if (ros.length() != 0) {
		// ros.deleteCharAt(ros.length() - 1);
		// }

		sql.append(" or (roles='' and departments='') ");

		sql.append(" or (roles='' and departments in (?)) ");
		for (int i = 0; i < deps.size(); i++) {

		}

		sql.append(" or (roles in (?) and departments =''  ) ");
		// parm.add(ros.toString());

		sql.append(" or (roles in (?) and departments in (?)) ");
		parm.add(deps.toString());
		// parm.add(ros.toString());

		System.out.println(sql.toString());
		System.out.println(parm.toArray().toString());
