...
/**
 * 模块：元数据管理 -> 数据来源管理 action层
 * 
 */
public class DataSrc extends BaseAction {
	/**
	 * 获取service层对象（cube）
	 * 
	 */
	private MetaServiceManager datasrcService = MetaService.getMetaService("ds");
	...
	public ModelAndView main() {
		...
		try {
			...
			list = datasrcService.QueryData_ByPage(null, where1, "sortcode", page.getPageNum() - 1, page.getPageSize());
			page.setData(list);
			int count = datasrcService.QueryCount(where1);
			page.setTotalRecorder(count);
			page.setUrl(sb.toString());

			// 如果下一页没有数据，就说明是最后一页
			List<Map<String, Object>> queryData_ByPage = datasrcService.QueryData_ByPage(null, where1, "sortcode", page.getPageNum(), page.getPageSize());
		...
		return new ModelAndView("/WEB-INF/jsp/metadata/datasrc/index").addObject("page", page).addObject("ismove", 1).addObject("bottom", bottom);
	}
	...
	public void query() throws IOException {
		...
		try {
			...
			List<Map<String, Object>> list = datasrcService.QueryData_ByPage(null, where1, "sortcode", page.getPageNum() - 1, page.getPageSize());
			int queryCount = datasrcService.QueryCount(where1);
			if (queryCount == 0) {
				List<SqlWhere> where2 = new ArrayList<SqlWhere>();
				if (!StringUtil.isEmpty(code)) {
					where2.add(new SqlWhere("code", code, 10));
				}
				list = datasrcService.QueryData_ByPage(null, where2, "sortcode", page.getPageNum() - 1, page.getPageSize());
				queryCount = datasrcService.QueryCount(where2);
			}
			page.setData(list);
	...
	public ModelAndView turnToAdd() {
		...
		CubeMetaTable cubeMetaTable = datasrcService.getMetaTable();
		...
		return new ModelAndView("/WEB-INF/jsp/metadata/datasrc/add").addObject("code", code).addObject("proname", proname);
	}
	...
	public void add() throws IOException {
		...
		String procode = req.getParameter("procode"); // 地区代码
		...
		if (StringUtil.isEmpty(id)) {
			try {
				int insertRow = datasrcService.InsertRow(codes);
				LogService.logOperation(req,UserService.getCurrentUser().getUserid(),LogConst.APPID, LogConst.COLUMN_DATASRC, LogConst.EXCSEC_INSERT, null, "" + insertRow, codes, code, procode, null, null, null, null, null, null, null, null);
				if (insertRow == 1) {
			...
			keys.put("code", code);
			try {
				int updateRow = datasrcService.UpdateRow(keys, codes);
				LogService.logOperation(req,UserService.getCurrentUser().getUserid(),LogConst.APPID, LogConst.COLUMN_DATASRC, LogConst.EXCSEC_UPDATE, null, "" + updateRow, codes, code, procode, null, null, null, null, null, null, null, null);
				if (updateRow == 1) {
					data.setReturncode(200);
					data.setParam1(code);
					data.setReturndata("操作成功");
		...
		if (!StringUtil.isEmpty(cname)) {
			list.add(new Synonyms("ds", "1", cname, code, Zbmgr.dbcode));
		}
		if (!StringUtil.isEmpty(ccname)) {
			list.add(new Synonyms("ds", "2", ccname, code, Zbmgr.dbcode));
		}
		...
		if (!StringUtil.isEmpty(synonym)) {
			...
			for (int i = 0; i < split.length; i++) {
				...
				list.add(new Synonyms("ds", "3", split[i], code, Zbmgr.dbcode));
			}
		}

		// 元数据操作成功 添加同义词
		if (200 == data.getReturncode()) {
			datasrcService.addSynonyms(list, code, "ds");
		}
	...
	public void delete() throws IOException {
		HttpServletRequest req = CurrentContext.getRequest();
		String code = req.getParameter("delId");
		code = code.trim();
		int result = 0;
		// 构造返回对象
		...
		try {
			...
			int queryCount2 = metaService3.QueryCount("datasource = '" + code + "'");
			if (queryCount2 > 0) {
			...
			// boolean queryTemplate = metaDataDao.queryTemplate("ds_" + code);
			// if (queryTemplate) {
			...
			// }
			// 任务表
			// boolean queryTask = metaDataDao.queryTask("ds_" + code);
			// if (queryTask) {
			...
			// }
			// 临时表
			// boolean queryTmpTable = metaDataDao.queryTmpTable(code, "datasource");
			// if (queryTmpTable) {
			...
			Map<String, Object> keys = new HashMap<String, Object>();
			keys.put("code", code);
			result = datasrcService.deleteRow(keys);
	...
	public void deleteAll() throws IOException {
		...
		try {
			for (int i = 0; i < ids.length; i++) {
				...
				// 底库
				MetaServiceManager metaService3 = MetaService.getMetaService("data");
				int queryCount2 = metaService3.QueryCount("datasource = '" + ids[i] + "'");
				if (queryCount2 > 0) {
				...
				// 模板表
				// MetaDataDaoImpl metaDataDao = new MetaDataDaoImpl();
				// boolean queryTemplate = metaDataDao.queryTemplate("ds_" + ids[i]);
				// if (queryTemplate) {
				...
				// }
				// 任务表
				// boolean queryTask = metaDataDao.queryTask("ds_" + ids[i]);
				// if (queryTask) {
				...
				// }
				// 临时表
				// boolean queryTmpTable = metaDataDao.queryTmpTable(ids[i], "datasource");
				// if (queryTmpTable) {
			...
			int updateRows = datasrcService.deleteRows(keys);
			if (updateRows != 0) {
				data.setReturncode(200);
				data.setReturndata("删除成功！");
				this.sendJson(data);
	...
	public ModelAndView turnToEdit() {
		...
		try {
			List<Map<String, Object>> queryData = datasrcService.QueryData(null, "code='" + code + "'", "sortcode");
			if (queryData.size() > 0) {
			...
			// 获取父节点名称
			if (!StringUtil.isEmpty(procode)) {
				DataTable tree = datasrcService.getTree(codes, "code='" + procode + "'", "sortcode");
		...
		// 查询同义词
		List<Synonyms> list = datasrcService.getSynonyms(code);
		CubeMetaTable cubeMetaTable = datasrcService.getMetaTable();
		if(cubeMetaTable != null){
			int codeLen = cubeMetaTable.getCodelen();
			if(codeLen == 0 ||codeLen>20){
		...
		return new ModelAndView("/WEB-INF/jsp/metadata/datasrc/edit").addObject(data).addObject("procodeName", procodeName).addObject("list", list).addObject("tyc", datasrcService.getSynonString(list)).addObject(data);
	}
	...
	public ModelAndView find() throws IOException, MetaTableException {
		...
		if (StringUtil.isEmpty(procode)) {
			...
			treeList = datasrcService.getTreePath(procode);
		...
		if (page.getPageNum() != 1) {
			List<Map<String, Object>> queryData_ByPage = datasrcService.QueryData_ByPage(null, where1, "sortcode", page.getPageNum() - 2, page.getPageSize());
			if (queryData_ByPage.size() >= 1) {
		...
		if (queryCount == 0 && !StringUtil.isEmpty(procode)) {
			...
			list = datasrcService.QueryData_ByPage(null, where1, "sortcode", page.getPageNum() - 1, page.getPageSize());
			queryCount = datasrcService.QueryCount(where1);
		}
		...
		try {
			...
			page.setTotalRecorder(queryCount);
			page.setUrl(pageUrl);
			LogService.logOperation(req, UserService.getCurrentUser().getUserid(),LogConst.APPID,LogConst.COLUMN_DATASRC, LogConst.EXC_QUERY, null, "" + queryCount, procode, procode, null, null, null, null, null, null, null, null, null);
		} catch (Exception e) {
			e.printStackTrace();
		}
		if (StringUtil.isEmpty(pjax)) {
			return new ModelAndView("/WEB-INF/jsp/metadata/datasrc/index").addObject("page", page).addObject("initTreePara", treeList).addObject("ismove", 1).addObject("top", top).addObject("bottom", bottom);
		} else {
			return new ModelAndView("/WEB-INF/jsp/metadata/datasrc/tableList").addObject("page", page).addObject("ismove", 1).addObject("top", top).addObject("bottom", bottom);
		}
	}
	...
	public ModelAndView quickFind() throws IOException, MetaTableException {
		...
		if (StringUtil.isEmpty(procode)) {
			list = datasrcService.QueryData_ByPage(null, where1, "sortcode", page.getPageNum() - 1, page.getPageSize());
			count = datasrcService.QueryCount(where1);
		} else {
			list = datasrcService.QueryData_InTree_ByPage(null, where1, "code", procode, "procode", "sortcode", page.getPageNum() - 1, page.getPageSize());
			count = datasrcService.QueryCount_InTree(where1, "code", procode, "procode");
		}
		...
		LogService.logOperation(req,UserService.getCurrentUser().getUserid(),LogConst.APPID, LogConst.COLUMN_DATASRC, LogConst.EXCSEC_FIND, null, "" + count, codes, code, cname, procode, null, null, null, null, null, null, null);
		} catch (Exception e) {
			e.printStackTrace();
		}
		if (StringUtil.isEmpty(pjax)) {
			return new ModelAndView("/WEB-INF/jsp/metadata/datasrc/index").addObject("page", page).addObject("code", code.replaceAll("%25", "%")).addObject("cname", cname.replaceAll("%25", "%")).addObject("initTreePara", treeList).addObject("searchPara", searchPara).addObject("ismove", 0);
		} else {
			return new ModelAndView("/WEB-INF/jsp/metadata/datasrc/tableList").addObject("page", page).addObject("ismove", 0);
		}
	}
	 ...
	 * @author chenyf
	 */
	public void findDatasrcTree() throws MetaTableException, IOException {
		HttpServletRequest req = CurrentContext.getRequest();
		HttpServletResponse resp = CurrentContext.getResponse();
		List<SqlWhere> where1 = new ArrayList<SqlWhere>();
		String code = req.getParameter("id");
		...
		// 取出状态为不删除的数据
		DataTable dt = datasrcService.getTree(null, where1, "sortcode");
	...
	public void checkCode() throws IOException {
		...
		try {
			int queryCount = datasrcService.QueryCount("code='" + code + "'");
			if (queryCount == 0) {
				data.setReturncode(200);
	...
	public void move() throws IOException {
		...
		code.add("sortcode");
		try {
			List<Map<String, Object>> queryData = datasrcService.QueryData(code, "code='" + currentCode + "'", null);
			if (queryData.size() > 0) {
				currentSortCode = (String) queryData.get(0).get("sortcode");
			}
			List<Map<String, Object>> queryData1 = datasrcService.QueryData(code, "code='" + siblingsCode + "'", null);
		...
		try {
			int updateRows = datasrcService.UpdateRows(keys, codes);
			LogService.logOperation(req,UserService.getCurrentUser().getUserid(),LogConst.APPID, LogConst.COLUMN_DATASRC, LogConst.EXCSEC_MOVE, null, "" + updateRows, cos, currentCode, siblingsCode, null, null, null, null, null, null, null, null);
			if (updateRows == 2) {
				data.setReturncode(200);
	...
	public ModelAndView getById() throws MetaTableException {
		...
		// 查询同义词
		List<Synonyms> list = datasrcService.getSynonyms(code);
	 LogService.logOperation(req,UserService.getCurrentUser().getUserid(),LogConst.APPID, LogConst.COLUMN_DATASRC, LogConst.EXC_QUERY_INFO, null, "" + queryCount, code, code, null, null, null, null, null, null, null, null, null);
		return new ModelAndView("/WEB-INF/jsp/metadata/datasrc/detail").addObject("proname", proname).addObject("list", list).addObject(data);
	}
	...
	public void exportData() throws IOException, MetaTableException {
		...
		List<Map<String, Object>> list = datasrcService.QueryData(codes, where1, "sortcode");
		int queryCount = datasrcService.QueryCount(where1);
		if (queryCount == 0) {
			where1.clear();
			if (!StringUtil.isEmpty(code)) {
			...
			list = datasrcService.QueryData(codes, where1, "sortcode");
		}
		 LogService.logOperation(req,UserService.getCurrentUser().getUserid(),LogConst.APPID, LogConst.COLUMN_DATASRC, LogConst.EXC_EXPORT2, null, "" + list.size(), code, procodeName, null, null, null, null, null, null, null, null, null);
		MetaDataExport.export(req, resp, list, getTitle(), getField(), procodeName, null, Const.DATASRC_FILE_NAME);
	}
	...
	public void exportSearchData() throws IOException, MetaTableException {
		...
		StringBuffer sb = new StringBuffer();
		if (!StringUtil.isEmpty(code)) {
			sb.append("数据来源代码=").append(code);
			where1.add(new SqlWhere("code", "%" + code + "%", 20));
		}
		if (!StringUtil.isEmpty(cname)) {
			sb.append("数据来源名称=").append(cname);
		...
		if (!StringUtil.isEmpty(procode)) {
			list = datasrcService.QueryData_InTree(codes, where1, "code", procode, "procode", "sortcode");
		...
		cod.put("procodeName", procodeName);
		 LogService.logOperation(req,UserService.getCurrentUser().getUserid(),LogConst.APPID, LogConst.COLUMN_DATASRC, LogConst.EXC_EXPORT3, null, "" + list.size(), codes, code, cname, procode, procodeName, null, null, null, null, null, null);
		MetaDataExport.export(req, resp, list, getTitle(), getField(), procodeName, sb.toString(), Const.DATASRC_FILE_NAME);
	}
	...
	public void mdExport() throws MetaTableException {
		...
		for (int i = 0; i < sqlList.size(); i++) {
			...
			List<SqlWhere> where1 = new ArrayList<SqlWhere>();
			where1.add(new SqlWhere(id, string, 16));
			list.addAll(datasrcService.QueryData(getField(), where1, null));
		}
		LogService.logOperation(req,UserService.getCurrentUser().getUserid(),LogConst.APPID, LogConst.COLUMN_DATASRC, LogConst.EXC_PLPP, null, "" + list.size(), sb.toString(), "" + list.size(), null, null, null, null, null, null, null, null, null);
		MetaDataExport.export(req, resp, list, getTitle(), getField(), null, null, "数据来源批量匹配数据结果", id);
	}
						...
						if (mkey.size() == getMust().size()) {
							...
							for (int i = 2; i < rows; i++) {
								...
								for (Iterator<Integer> iterator = mkey.keySet().iterator(); iterator.hasNext();) {
									...
									String value = datasrcService.getValue(cell.getText() + "");
									if (StringUtil.isEmpty(value)) {
							...
							if (flag && checkData && !(cqCode.size() > 0)) {
								...
								for (int i = 0; i < code.size(); i++) {
									...
									// 检查code
									int queryCount = datasrcService.QueryCount("code='" + cd + "'");
									if (queryCount > 0) {
								...
								if (cqCode.size() > 0) {
									...
									for (String key : keySet) {
										// 检查code
										int queryCount = datasrcService.QueryCount("code='" + needCheckProcode.get(key) + "'");
										if (queryCount == 0) {
									...
									if (checkProcode.size() > 0) {
										...
										int insertRows = datasrcService.InsertRows(sql);
	...
	public void exportAllData() throws IOException, MetaTableException {
		...
		if (StringUtil.isEmpty(codes)) {
			MetaDataExport.export(req, resp, new ArrayList<Map<String, Object>>(), getTitle(), getField(), null, null, Const.DATASRC_FILE_NAME);
			return;
		}
		...
		for (int i = 0; i < ids.length; i++) { // 最多遍历10次，1w条数据
			List<SqlWhere> where1 = new ArrayList<SqlWhere>();
			// 最多1000条数据
			List<Map<String, Object>> dt = datasrcService.QueryData_InTree(getField(), where1, "code", ids[i], "procode", "sortcode");
			for (int j = 0; j < dt.size(); j++) {
				list.add(dt.get(j));
			}
		}
		 LogService.logOperation(req,UserService.getCurrentUser().getUserid(),LogConst.APPID, LogConst.COLUMN_DATASRC, LogConst.EXC_EXPORT1, null, "" + ids.length, ids, "" + list.size(), null, null, null, null, null, null, null, null, null);
		MetaDataExport.export(req, resp, list, getTitle(), getField(), null, null, Const.DATASRC_FILE_NAME);
	}
	...
	public void checkProCode() throws IOException {
		...
		try {
			if (!StringUtil.isEmpty(procode)) {
				...
				int queryCount = datasrcService.QueryCount("code='" + clearCode + "'");
				if (queryCount != 0) {
					List<Map<String, Object>> queryData_TreePath = datasrcService.QueryData_TreePath(null, "code", clearCode, "procode");
					...
					if (queryData_TreePath.size() > 0) {
						...
						if (contains) {
							...
							data.setReturncode(200);
							List<Map<String, Object>> queryData = datasrcService.QueryData(null, "code='" + clearCode + "'", "sortcode");
