define(function(require, exports, module) {
	...
	$(document).on("click", "#saveMemo", function() {
		...
		$.ajax({
			url: rootPath + "/templatecenter/calCheckMemo.htm?m=madd&modId=" + modId,
			type: "post",
			data: {
	...
	function getMemoList() {
		$.ajax({
			url: rootPath + "/templatecenter/calCheckMemo.htm?m=mfind&modId=" + modId + "&templateCode=" + window.SPREADSHEET_AUTHENTIC_KEY,
			type: "post",
			dataType: "json",
			success: succ
		});
	...
	function removeMemoList(memoId) {
		$.ajax({
			url: rootPath + "/templatecenter/calCheckMemo.htm?m=mdel&modId=" + modId + "&templateCode=" + window.SPREADSHEET_AUTHENTIC_KEY + "&memoId=" + memoId,
			type: "post",
			dataType: "json",
			success: function(data) {
				if (data.returncode === 200) {
