            ...
            dropList: function (json, options, callback) {
                ...
                that.append(
                    ...
                    create('div', 'dtHtml').append(
                        create('div', 'dtHead').attr({
                            'node': JSON.stringify(json.nodes[n]),
                            'title': json.nodes[n].name
                        }).html(json.nodes[n].name),
                ...
                if (options.setItem != '') {
                    $('.dtHead', that).attr({
                        'node': '{"code":"' + options.setItem + '","name":"' + options.setItem + '","sort":' + 4 + '}',
                        'title': options.setItem
                    }).html(options.setItem);
                }

                $.each(json.nodes, function (i) {
                    $('.dtBody ul', that).append(
                        create('li').attr({
                            'node': JSON.stringify(json.nodes[i]),
                            'code': json.nodes[i].code,
                            'title': json.nodes[i].name
                        }).html(json.nodes[i].name)
                    )
                });
                ...
                $(that).on('click', '.dtTextBtn', function () {
                    ...
                    if (value.replace(/(^\s*)|(\s*$)/g, '') != '' && value != '时间') {
                        ...
                        if (flag) {
                            $('.dtHead', that).attr({
                                'node': '{"code":"' + value + '","name":"' + value + '","sort":' + 4 + '}',
                                'title': value
                            }).html(value);
                            $('.dtText', that).val('时间');
                            $('.dtBody', that).hide();
...
//					$('.dtBody',that).hide();
            ...
            addItem: function (index, node) {
                ...
                if ($('.dtBody ul li', this).length == 1) {
                    $('.dtHead', this).attr({
                        'node': $('.dtBody ul li:eq(0)', this).attr('node'),
                        'title': $('.dtBody ul li:eq(0)', this).html()
                    }).html($('.dtBody ul li:eq(0)', this).html());
                }
