define(function(require, exports, module) {
	...
	$(document).on('click', '.J_submit', function(e) {
		...
		for(i=0;i<len;i++){
			if($.trim($(elementlist[i]).children('td:eq(2)').html()) !== ''){
				dimensionList.push({
					type: $(elementlist[i]).find('td:eq(1) select option:selected').text(),
	...
	$(document).on('click', '.add_list_item', function(){
		...
		strHTML+='<tr><td>'+ (length+1) +'</td><td><select><option value ="zb">指标</option><option value ="fl">分组</option><option value="unit">计量单位</option>';
		strHTML+='<option value="tt">时间类型</option><option value="sj">时间</option><option value="ds">数据来源</option>';
		strHTML+='<option value="reg">地区管理</option><option value="reg">主体</option></select></td><td class="J_edit_code"></td><td></td>';
	 ...
	 * 编辑列表
	 */
	$(document).on('click', '.J_edit_code' ,function(event){
		var self =$(this);
	...
	$(document).on('blur','.J_editing_input',function(){
		var self = $(this),
			nameNode = self.parent().next(),
			dimensionNode = self.parent().prev(),
			value;
		...
		$.ajax({
            url: common.rootPath+ '/system/handle.htm?m=getDimensionName',
            type: 'post',
            timeout: 10000,
            dataType: 'json',
            data: 'dimension=' +dimensionNode.children('select').val() + '&code='+value,
            success: function(data) {
            	if(data!==null && data.returncode === 200){
            		nameNode.html(data.returndata);
            	}else{
            		nameNode.html('');
            	}
            }
