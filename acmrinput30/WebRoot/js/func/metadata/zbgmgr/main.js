define(function(require,exports,module){
	...
	var setting = {
		async: {
			enable: true,
			url: common.rootPath+'metadata/zbgmgr.htm?m=findZbTree&st='+st,
	...
	function clickEvent(event, treeid, treeNode) {
		//queryData(common.rootPath+"metadata/zbgmgr.htm?m=query", common.formatData('id',treeNode.id));
		...
		$.pjax({
			url: common.rootPath+"metadata/zbgmgr.htm?m=query"+str+ common.formatData('id',treeNode.id),
			container: '.J_zbgmgr_data_table'
		});
		$(document).on('pjax:success', function() {
		  delIds = [];
		});
	}
	var rootNode = [{"id":"","name":"指标分组树", "open":"true", "isParent":"true"}];
	...
	$(document).on('click','.J_grade_search',function(event){
		...
		var modalContent = '<div class="modal-header">';
			...
			modalContent +='<option value="ccname">中文全称</option><option value="ccname_en">英文全称</option><option value="ifdata">指标、分类</option><option value="unitcode">单位</option><option value="dotcount">小数点位数</option>';
...
//			common.commonTips("格式错误");
		...
		currentVal = $(".J_hpriview_sentence").val();
		if(currentVal === ''){
			return;
		}
		$.ajax({
			...
			type:'post',
			dataType:'json',
			data:'m=advQuery&searchSQL='+currentVal,
			timeout:5000,
			success:function(data){
				if(data.returncode == 200){
//					var returndata = data.returndata;
//					formatBackTable(data);	// page数据作为参数
//					new Pagination({
...
//					common.commonTips("格式错误");
		...
		$.pjax({
			url: common.rootPath+"metadata/zbgmgr.htm?m=advQuery&searchSQL="+currentVal,
			container: '.J_zbgmgr_data_table'
		});
		$(document).on('pjax:success', function() {
	...
	function fileUpload(){
		$('#J_import_fileupload',document).fileupload({
			url: common.rootPath+'metadata/zbgmgr.htm?m=upload',
			type: 'post',
	        dataType: 'json',
	 ...
	 *提交选择的checkebox(默认所有)
	 */
	$(document).on('click','.J_zbgmgr_table>thead>tr>th>input[type="checkbox"]',function(){
		var self = this;
		delIds = common.checkAll('.J_zbgmgr_table>tbody input[type="checkbox"]',$(self).prop('checked'),delIds);
	});
	$(document).on('click','.J_zbgmgr_table>tbody input[type="checkbox"]',function(){
		var self = this,
		...
		if(delIds.length === $('.J_zbgmgr_table>tbody input[type="checkbox"]').length){
			$('.J_zbgmgr_table>thead>tr>th>input[type="checkbox"]').attr('checked',true);
		}else{
			$('.J_zbgmgr_table>thead>tr>th>input[type="checkbox"]').attr('checked',false);
		}
	});
	...
	$(document).on('click','.J_opr_del',function(event){
		...
		$.ajax({
			url:common.rootPath+'metadata/zbgmgr.htm?m=toDelete',
			data:"delId=" + delId,
			type:'post',
			dataType:'json',
			timeout:10000,
	...
	$(document).on('click','.J_All_del',function(event){
		...
		$.ajax({
			url:common.rootPath+'metadata/zbgmgr.htm?m=toDeleteAll',
			data:"ids=" + delIds,
			type:'post',
			dataType:'json',
			timeout:10000,
	...
	$(document).on('click','.J_opr_moveup',function(event){
		...
		$.ajax({
			...
			success:function(data){
				if(data.returncode == 200){
					common.cancalCheck('.J_zbgmgr_table input[type="checkbox"]',delIds);
//					var tempHTML = '';
...
//						$(self).parents('tbody').children('tr:eq(0)').before(tempHTML);
//					}
//					else if(indexVal > 1 && indexVal< $('.J_zbgmgr_table tbody').children('tr').length-1){
//						$(self).parents('tbody').children('tr:eq('+(indexVal-2)+')').after(tempHTML);
//					}
//					else if(indexVal==$('.J_zbgmgr_table tbody').children('tr').length-1){
//						var spHTML='',
//						HTML='',
//						targetHTML='';
...
//					$(self).parents('tr:eq(0)').remove();
	...
	$(document).on('click','.J_export_data',function(event){
		...
		if(delIds.length>0){//有没有被选中的指标
			window.location.href=common.rootPath+"metadata/zbgmgr.htm?m=exportAllData&ids="+delIds;
		}else if(isMove){//是否为普通的数据查看
			window.location.href=common.rootPath+"metadata/zbgmgr.htm?m=exportData&id="+treeNodeId+"&procodeName="+treeNodeName;
		}else if(isAdvQuery){//是否为高级查询
			var hcurrentVal = $(".J_hpriview_sentence").val();
			window.location.href=common.rootPath+"metadata/zbgmgr.htm?m=exportAdvQueryData&searchSQL="+hcurrentVal;
		}else{//表示进入搜索
			if(searchField.length>0){
				searchPara = searchField;
			}
			window.location.href=common.rootPath+"metadata/zbgmgr.htm?m=exportSearchData&procodeName="+treeNodeName+searchPara;
		}
	});
	...
	function initFileUpload(){
		$('#J_fileupload',document).fileupload({
			url: common.rootPath+'metadata/zbgmgr.htm?m=matchingDetails',
	        dataType: 'json',
	        done: function (e, data) {
	...
	$(document).on('click','.J_btn_md_export',function(event){
		event.preventDefault();
		window.location.href=common.rootPath+"metadata/zbgmgr.htm?m=mdExport&id="+idColumn+"&uuid="+uuid;
	});
	...
	$(document).on('click', '#mouldDownload', function(event) {
		window.location.href = common.rootPath+"metadata/zbmgr.htm?m=templateDownload";
	});
	...
	$(document).on('click', '#mouldDownload2', function(event) {
		window.location.href = common.rootPath+"metadata/zbmgr.htm?m=templateDownload2";
	});
	...
	function importData(){
		$('#J_fileupload_import',document).fileupload({
			url: common.rootPath+'metadata/zbgmgr.htm?m=importData',
	        dataType: 'json',
	        done: function (e, data) {
	        	var result = data.result;
	            if(result.returncode == 200){
	...
	$(document).on('click','.J_btn_id_export',function(event){
		event.preventDefault();
		window.location.href=common.rootPath+"metadata/zbgmgr.htm?m=impErrInfoExport&uuid="+uuid;
	});
	...
	$(document).on('click','.J_Add',function(event){
		event.preventDefault();
		window.location.href=common.rootPath+"metadata/zbgmgr.htm?m=toAdd&procodeId=" + treeNodeId + "&procodeName=" + treeNodeName;
	});
	/**
	 *根据code下转 
	 */
	$(document).on('click','.J_opr_code',function(event){
		...
		var flag = false;
		$.ajax({
			url:common.rootPath+"metadata/zbgmgr.htm?m=checkHasParent&code="+currentCode,
			type:'post',
			timeout:5000,
			dataType:'json',
			async: false,
		...
		$.pjax({
			url: common.rootPath+"metadata/zbgmgr.htm?m=query&id="+currentCode,
			container: '.J_zbgmgr_data_table'
		});
		$(document).on('pjax:success', function() {
		  delIds = [];
	...
	function formatBackTable(data){
		if(data.returncode == 200){
			...
			for (i; i < len; i++) {
				...
				trs +='<td><a href="'+common.rootPath+'metadata/zbgmgr.htm?m=getDataById&id='+list[i].code+'" >'+list[i].cname+'</a></td>';
				trs +='<td>'+list[i].unitcode+'</td>';
				trs +='<td>'+list[i].procode+'</td>';
				trs +='<td>'+list[i].ccname+'</td>';
				trs +='<td><a href="'+common.rootPath+'metadata/zbgmgr.htm?m=toEdit&id='+list[i].code+'" class="btn-opr J_opr_edit">编辑</a>';
	 ...
	 * 翻页操作
	 */
	$(document).pjax('.J_zbgmgr_pagination a', '.J_zbgmgr_data_table');
