define(function(require, exports, module) {
	...
	doc.on('submit', '.J_edit_form', function(event) {
		...
		//前端检查
		if (!checkDelegate.checkNormal($('input[name="cname"]'), [{'name': 'required','msg': '中文名称不能为空'},{'name':'maxlength','param':51,'msg':'中文名称长度不能超过50个字符'}]) || 
	...
	function checkProcodeAsyncExist(currentVal,checkDelegate){
		var flag = false;
		$.ajax({
			url: common.rootPath+'metadata/regmgr.htm?m=checkProCode',
			timeout: 5000,
			type: 'post',
			async: false,
