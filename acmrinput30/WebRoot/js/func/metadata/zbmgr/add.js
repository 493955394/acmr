define(function(require, exports, module) {
	...
	doc.on('submit', '.J_add_form', function(event) {
		...
		//前端检查
		if (!checkDelegate.checkNormal($('input[name="code"]'), [{'name': 'required','msg': '指标代码不能为空'},{'name': 'ch','msg': '指标代码不能包含中文'},{'name':'maxlength','param':21,'msg':'指标代码长度不能超过20个字符'},{'name':'minlength','param':0,'msg':'指标代码长度不能少于1个字符'}]) ||
...
//		}
	 ...
	 * 获取随机码
	 */
	$(document).on('click','input[id="getCodeBtn"]',function(){
		var self = this;
		$.ajax({
			url: common.rootPath+"metadata/zbmgr.htm?m=getCode",
			type: 'get',
			async: false,
			dataType: 'json',
			success: function(data) {
				if(data.returncode == 200){
					$('input[name="code"]').val(data.returndata);
				}
			},
			error: function(e) {
				common.commonTips('操作失败');
