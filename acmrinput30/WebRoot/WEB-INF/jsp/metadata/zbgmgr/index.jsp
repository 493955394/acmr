...
<html>
	...
	<body>
		...
		<div class="container-fluid" id="mainpanel">
			<div class="col-md-2 left-panel">
			    <div class="panel tree-panel">
			        <div class="panel-heading">指标分组树</div>
			    </div>
				...
				<div class="panel panel-default">
					...
					<div class="panel-body">
						<div class="toolbar-left">
							<form class="form-inline J_search_form" action="${ctx}/metadata/zbgmgr.htm?m=find">
								<div class="form-group">
									<select id="querykey" class="form-control input-sm">
										<option value="code" <c:if test="${code != '' && code!= null}">selected</c:if>>指标分组代码</option>
										<option value="cname" <c:if test="${cname != '' && cname != null}">selected</c:if>>指标分组名称</option>
									</select>
								</div>
								<div class="form-group">
					...
					<div class="J_zbgmgr_data_table">
						<jsp:include page="/WEB-INF/jsp/metadata/zbgmgr/tableList.jsp" flush="true"/>
					</div>
					<!-- 
					<div class="toolbar-right">
			            <ul class="pagination">
			            	...
			            	<c:forEach begin="2" end="4" varStatus="i">
				            	<c:if test="${page.pageNum+i.index-1 <= page.totalPage}">
				            		<li><a href="${ctx}/zbgmgr.htm?m=query&pageNum=${page.pageNum+i.index-1}">${i.index}</a></li>
								</c:if>
			            	</c:forEach>
			            	<c:choose>
			            		...
			            		<c:otherwise>
					            	<li><a href="${ctx}/zbgmgr.htm?m=query&pageNum=${page.pageNum+1}">下一页</a></li>
					            	<li><a href="${ctx}/zbgmgr.htm?m=query&pageNum=${page.totalPage}">末页</a></li>
			            		</c:otherwise>
			            	</c:choose>
			            </ul>
			        </div>
		...
		<script type="text/javascript">
			seajs.use('${ctx}/js/func/metadata/zbgmgr/main');
		</script>
	</body>
</html>