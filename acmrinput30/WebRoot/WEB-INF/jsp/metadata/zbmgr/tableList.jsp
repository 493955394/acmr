	...
	<tbody>
		...
		<c:forEach items="${page.data}" var="list" varStatus="status">
			<tr>
				...
				<td><a href="${ctx}/metadata/zbmgr.htm?m=getDataById&id=${list.code}" >${list.cname}</a></td>
				<td>${list.unitcode}</td>
				<td>${list.procode}</td>
				<td>${list.ccname}</td>
				<td>
					<a href="${ctx}/metadata/zbmgr.htm?m=toEdit&id=${list.code}" class="btn-opr J_opr_edit">编辑</a>
