...
<table class="table table-striped table-hover J_datetype_table">
	<colgroup>
		<col width="3%"/>
		<col width="5%"/>
		<col width="12%"/>
	...
	<tbody>
		...
		<c:forEach items="${page.data}" var="list" varStatus="status">
			<tr>
				...
				<td><a href="${ctx}/metadata/datetype.htm?m=getById&id=${list.code}" >${list.cname}</a></td>
				<td>${list.unitcode}</td>
				<td>${list.procode}</td>
				<td>${list.ccname}</td>
				<td>
					<a href="${ctx}/metadata/datetype.htm?m=turnToUpdate&id=${list.code}" class="btn-opr J_opr_edit">编辑</a>
...
<div class="toolbar-right">
	<ul class="pagination J_datetype_pagination">${page}</ul>  	
</div>