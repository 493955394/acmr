...
<html>
	<head>
		<meta charset="UTF-8">
		<title>${projectTitle}-时间类型管理</title>
	...
	<body>
		...
		<div class="container-fluid" id="mainpanel">
			<div class="col-md-2 left-panel">
			    <div class="panel tree-panel">
			        <div class="panel-heading">时间类型树</div>
			    </div>
				...
				<div class="panel panel-default">
					...
					<div class="panel-body">
						<div class="toolbar-left">
							<form class="form-inline J_search_form" action="${ctx}/metadata/datetype.htm?m=quickFind">
								<div class="form-group">
									<select id="querykey" class="form-control input-sm">
										<option value="code" <c:if test="${code != '' && code!= null}">selected</c:if>>时间类型代码</option>
										<option value="cname" <c:if test="${cname != '' && cname != null}">selected</c:if>>时间类型名称</option>
									</select>
								</div>
								<div class="form-group">
						...
						<div class="toolbar-right">
							<div class="toolbar-group" style="position: relative;">
								...
								<a href="" class="btn btn-default btn-sm J_All_del">批量删除</a>
							</div>
						</div>
					</div>
					
					<div class="J_datetype_data_table">
						<jsp:include page="/WEB-INF/jsp/metadata/ttmgr/tableList.jsp" flush="true"/>
					</div>
					<%--<div class="toolbar-right">
			            <ul class="pagination">
			            	...
			            	<c:forEach begin="2" end="4" varStatus="i">
				            	<c:if test="${page.pageNum+i.index-1 <= page.totalPage}">
				            		<li><a href="${ctx}/datetype.htm?m=query&pageNum=${page.pageNum+i.index-1}">${i.index}</a></li>
								</c:if>
			            	</c:forEach>
			            	<c:choose>
			            		...
			            		<c:otherwise>
					            	<li><a href="${ctx}/datetype.htm?m=query&pageNum=${page.pageNum+1}">下一页</a></li>
					            	<li><a href="${ctx}/datetype.htm?m=query&pageNum=${page.totalPage}">末页</a></li>
			            		</c:otherwise>
			            	</c:choose>
			            </ul>
			        </div>
		...
		<script type="text/javascript">
			seajs.use('${ctx}/js/func/metadata/ttmgr/main');
		</script>
	</body>
</html>