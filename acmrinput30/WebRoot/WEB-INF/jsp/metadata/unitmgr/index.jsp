...
<html>
	...
	<body>
		...
		<div class="container-fluid" id="mainpanel">
			<div class="col-md-2 left-panel">
			    <div class="panel tree-panel">
			        <div class="panel-heading">计量单位树</div>
			    </div>
				...
				<div class="panel panel-default">
					...
					<div class="panel-body">
						<div class="toolbar-left">
							<form class="form-inline J_search_form" action="${ctx}/metadata/unit.htm?m=quickFind">
								<div class="form-group">
									<select id="querykey" class="form-control input-sm">
										<option value="code" <c:if test="${code != '' && code!= null}">selected</c:if>>计量单位代码</option>
										<option value="cname" <c:if test="${cname != '' && cname != null}">selected</c:if>>计量单位名称</option>
									</select>
								</div>
								<div class="form-group">
					...
					<div class="J_unit_data_table">
						<jsp:include page="/WEB-INF/jsp/metadata/unitmgr/tableList.jsp" flush="true"/>
					</div>
			            ...
			            <ul class="pagination">
			            	...
			            	<c:forEach begin="2" end="4" varStatus="i">
				            	<c:if test="${page.pageNum+i.index-1 <= page.totalPage}">
				            		<li><a href="${ctx}/unit.htm?m=query&pageNum=${page.pageNum+i.index-1}">${i.index}</a></li>
								</c:if>
			            	</c:forEach>
			            	<c:choose>
			            		...
			            		<c:otherwise>
					            	<li><a href="${ctx}/unit.htm?m=query&pageNum=${page.pageNum+1}">下一页</a></li>
					            	<li><a href="${ctx}/unit.htm?m=query&pageNum=${page.totalPage}">末页</a></li>
			            		</c:otherwise>
			            	</c:choose>	
			            </ul>
			        </div>
		...
		<script type="text/javascript">
			seajs.use('${ctx}/js/func/metadata/unitmgr/main');
		</script>
	</body>
</html>